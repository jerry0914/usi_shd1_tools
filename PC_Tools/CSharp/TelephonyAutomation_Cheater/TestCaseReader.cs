            ...
            try
            {
                ...
                for (; currentRow < workSheet1.Cells.Rows.Count; currentRow++)
                {
                    try
                    {
                        ...
                        {
                            tcNew.CurrentFormat = Wwan_TestCaseInfo.Format.TD_SCDMA;
                        }
                        else if(tcNew.Name.Contains("UMTS")||tcNew.Name.Contains("WCDMA"))
                        {
                            tcNew.CurrentFormat = Wwan_TestCaseInfo.Format.WCDMA;
                        }
                        else if (tcNew.Name.Contains("LTE") || tcNew.Name.Contains("4G"))
                        {
                            tcNew.CurrentFormat = Wwan_TestCaseInfo.Format.LTE;
                        }
                        else
                        {
                            tcNew.CurrentFormat = Wwan_TestCaseInfo.Format.GSM;
                        }
